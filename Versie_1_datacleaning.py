...
def count_keywords(query):
    ...
    # Check for combination of 'group by' and 'having'
...
queries_df = queries_df[~queries_df['Query Name'].isin(['3_5', '4_6', '4_7'])]

# Group by relevant columns and get the minimum timestamp for each group
grouped_df = queries_df.groupby(['Folder', 'File','Query Name', 'Query', 'Error', 'Contains error', 'Error Type', 'Bracket Count', 'Table Count', 'Where Count',  'Group By Count', 'Join Count', 'Having count', 'Distinct count', 'Group By & Having']).agg({'Timestamp': 'min'}).reset_index()
